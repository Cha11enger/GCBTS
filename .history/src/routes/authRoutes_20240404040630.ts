...
const handleGitHubCallback = async (req: Request, res: Response) => {
    ...
    try {
        const { access_token, token_type, scope } = await exchangeCodeForToken(req.query.code as string);
        
        // Now, use access_token to fetch user info and save it...
        const userResponse = await axios.get('https://api.github.com/user', { headers: { Authorization: `token ${access_token}` } });
        const userData = userResponse.data;
        
        // Assuming saveUser is a utility function that takes user data and saves it:
        await saveUser({
            githubId: userData.id,
            username: userData.login,
            email: userData.email, // GitHub's user email might not be directly available; you might need to handle this.
...
const saveUser = async (username: string, email: string, avatar_url: string, access_token: string) => {
    try {
