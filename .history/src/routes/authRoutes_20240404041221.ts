...
const handleGitHubCallback = async (req: Request, res: Response) => {
    ...
    try {
        const tokenData = await exchangeCodeForToken(req, res); // Adjusted to pass request and response objects
        const access_token = tokenData;

        const userResponse = await axios.get('https://api.github.com/user', { headers: { Authorization: `token ${access_token}` } });
        const userData = userResponse.data;

        await saveUser(userData.login, userData.email, userData.avatar_url,  userData.access_token, userData. // Adjusted to pass direct arguments
