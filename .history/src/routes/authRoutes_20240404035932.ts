...
const handleGitHubCallback = async (req: Request, res: Response) => {
    ...
    try {
        ...
        const tokenResponse = await exchangeCodeForToken(req, res);
        ...
        const { access_token } = tokenResponse.data.response; // Access the access_token property from the response property of tokenResponse

        // get the user information from the github api and save it to the database
        const userResponse = await axios.get('https://api.github.com/user', {
            headers: {
                Authorization: `token ${access_token}` // Use the access_token variable here
            }
        });

        const { login: username, email, avatar_url } = userResponse.data;
        console.log('User information:', { username, email, avatar_url });

        // Save user to the database
        await saveUser(username, email); // Pass the required arguments to the saveUser function
